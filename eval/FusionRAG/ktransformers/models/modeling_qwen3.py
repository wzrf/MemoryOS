...
#
# This code is based on the original Qwen3 implementation and has been modified
# to support FusionRAG and other custom functionalities.
#
...
_CHECKPOINT_FOR_DOC = "Qwen/Qwen3-32B"
...
def eager_attention_forward(
    ...
    **kwargs,
    ...
    # FusionRAG importance cache computation
    if kwargs.get('reprocess_method') == 'FusionRAG':
        past_key_value = kwargs.get('past_key_value')
        if past_key_value is not None and module.layer_idx == module.config.num_hidden_layers - 1:
            ...
            for context_id, context_len in enumerate(passages_len[:-1]):
                ...
                context_key = history_key_cache[context_id].to(query.device)[module.layer_idx]
                context_key = repeat_kv(context_key, module.num_key_value_groups)
                context_key = context_key.transpose(-1, -2)
                attn_weights_importance = torch.matmul(query, context_key)
                attn_weights_importance /= math.sqrt(module.head_dim)
                attn_weights_importance = nn.functional.softmax(attn_weights_importance, dim=-1, dtype=torch.float16)
                attn_weights_importance = torch.sum(torch.sum(attn_weights_importance, dim=0), dim=-2)
                assert not torch.isinf(attn_weights_importance).any()
                assert not torch.isnan(attn_weights_importance).any()
                assert context_len == attn_weights_importance.shape[1]
                past_key_value.importance_cache[module.layer_idx].narrow(1, past_len, context_len).copy_(attn_weights_importance)
            ...
            if kwargs['reprocess_method'] == 'FusionRAG':
                ...
                if self.layer_idx == self.config.num_hidden_layers - 1:
                    for context_id, context_len in enumerate(passages_len[:-1]):
                        ...
                        context_key = context_key.transpose(-1, -2)
                        attn_weights_importance = torch.matmul(query_states, context_key)
                        attn_weights_importance /= math.sqrt(self.head_dim)
                        attn_weights_importance = nn.functional.softmax(attn_weights_importance, dim=-1, dtype=torch.float16)
                        attn_weights_importance = torch.sum(torch.sum(attn_weights_importance, dim=0), dim=-2)
                        assert not torch.isinf(attn_weights_importance).any()
                        assert not torch.isnan(attn_weights_importance).any()
                        assert context_len == attn_weights_importance.shape[1]
                        past_key_value.importance_cache[self.layer_idx].narrow(1, past_len, context_len).copy_(attn_weights_importance)
