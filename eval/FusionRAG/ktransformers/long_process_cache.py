...
def prepare_data(model_name, data_path, data_name, cache_path, tokenizer: AutoTokenizer, topk: int, revert_rope, preprocess, random_flag):

    prompt_config = json.load(open('/mnt/data/benchmark/config/dataset2prompt_few-shot.json'))
    data_path = data_path+data_name
    if data_name in ['2wikimqa.jsonl', 'samsum.jsonl', 'multi_news.jsonl', 'musique.jsonl','hotpotqa.jsonl']:
    ...
    if not os.path.exists(f"{cache_path}data/{data_name.split('.')[0]}_{random_flag}/{model_name}"):
        os.makedirs(f"{cache_path}data/{data_name.split('.')[0]}_{random_flag}/{model_name}")
    if not os.path.exists(f"{cache_path}{data_name.split('.')[0]}_{random_flag}/{model_name}"):
        os.makedirs(f"{cache_path}{data_name.split('.')[0]}_{random_flag}/{model_name}")
    if not os.path.exists(f"{cache_path}data/{data_name.split('.')[0]}-preprocess-{topk}-{revert_rope}_{random_flag}/{model_name}"):
        os.makedirs(f"{cache_path}data/{data_name.split('.')[0]}-preprocess-{topk}-{revert_rope}_{random_flag}/{model_name}")
    if not os.path.exists(f"{cache_path}{data_name.split('.')[0]+f'-preprocess-topk{topk}'}_{random_flag}"):
        os.makedirs(f"{cache_path}{data_name.split('.')[0]+f'-preprocess-topk{topk}'}_{random_flag}")
    if not os.path.exists(f"{cache_path}{data_name.split('.')[0]+f'-preprocess-topk{topk}_{random_flag}'}/{model_name}"):
        os.makedirs(f"{cache_path}{data_name.split('.')[0]+f'-preprocess-topk{topk}'}_{random_flag}/{model_name}")
    ...
    return batch_data, batch_tokens,question_list, real_answer_list, stop_token_id, \
        f"{cache_path}data/{data_name.split('.')[0]}_{random_flag}/{model_name}", f"{cache_path}data/{data_name.split('.')[0]}-preprocess-{topk}-{revert_rope}_{random_flag}/{model_name}", \
...
def main(model_path= '/mnt/data/model/Mistral-7B-Instruct-v0.3', 
    ...
    # prepare data
    prompt_data, tokens_data, question_list, real_answer_list, stop_token_id, save_path, preporcess_save_path, data_name_prefix, rouge_metrics, context_rank, corpus_lens  = prepare_data(model_name, data_path, data_name, cache_path, tokenizer, topk, revert_rope, preprocess, random_flag)
   # preprocess preprare topk
    ...
    if preprocess:
        csv_file = f"{cache_path}{data_name.split('.')[0]}-preprocess-topk{topk}_{random_flag}/{model_name}/reprocess_method_{reprocess_method}_rate_{rate}_dense_{dense}_topk_{topk}_revert_rope_{revert_rope}_random_flag{random_flag}.csv"
    else:
        csv_file = f"{cache_path}{data_name.split('.')[0]}_{random_flag}/{model_name}/reprocess_method_{reprocess_method}_rate_{rate}_dense_{dense}_topk_{topk}_revert_rope_{revert_rope}_random_flag_{random_flag}.csv"
    ...
    if preprocess:
        file_path = f"{cache_path}{data_name.split('.')[0]+f'-preprocess-topk{topk}'}/{model_name}_reprocess_method_{reprocess_method}_rate_{rate}_topk_{topk}_dense{dense}_revert_rope_{revert_rope}_random_flag_{random_flag}.txt"
    else:
        file_path = f"{cache_path}{data_name.split('.')[0]}/{model_name}_reprocess_method_{reprocess_method}_rate_{rate}_topk_{topk}_dense{dense}_revert_rope_{revert_rope}_{random_flag}.txt"
...
# data_name = 'musique-200.jsonl'
# for rate in [0,0.05,0.1,0.15,0.2,0.3,0.4,0.5,1]:
#     main(rate = rate, preprocess=False, revert_rope=False, reprocess_method='cacheBlend',data_name=data_name, random_flag=False) 
#     main(rate = rate, preprocess=False, revert_rope=False, reprocess_method='processCache',data_name=data_name, random_flag=False) 
    # main(rate = rate, preprocess=False, revert_rope=False, reprocess_method='cacheBlend') 
for rate in [0,0.05,0.1,0.15,0.2,0.3,0.4,0.5,1]:
    main(rate = rate, preprocess=True, revert_rope=False, reprocess_method='processCache',data_name=data_name, random_flag=False, topk = 5) 